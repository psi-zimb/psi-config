...
from
 ...
 on obsForSecondLineTreatment.person_id = pat.patient_id
        ...
        WHERE
        pat.patient_id not in (/*Remove patient who's last stage in the reporting period is not second line.*/
          Select ifnull(case when concept_view.concept_full_name != 'PR, 2nd Line' then obs.person_id END,0) from
                     obs join concept_view
                     on obs.value_coded = concept_view.concept_id
                      where obs.obs_group_id in
                     (
                         Select max(obs_group_id) from obs where concept_id in
                         (
                         select concept_id from concept_name where name ='PR, Start date of ART Stage'
                         )
                         and value_datetime between date('#startDate#') and date('#endDate#')
                         and voided=0
                         GROUP BY obs.person_id
                      )
                         and obs.concept_id in (select concept_id from concept_name where name='PR, ART Stage') and voided =0
                         GROUP BY obs.person_id
        )
        AND
         obsForSecondLineTreatment.dateTimeForSecLine between date('#startDate#') and date('#endDate#')
        group by
            obsForSecondLineTreatment.dateTimeForSecLine,pat.patient_id;
