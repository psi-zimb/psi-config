...
case
    ...
    when person.gender = 'O' then 'Other'
    end as "Sex",
TIMESTAMPDIFF(YEAR, person.birthdate, CURDATE()) as Age,
GROUP_CONCAT(DISTINCT (case when personAttributeType.name = 'Population' then cvForRefferedFrom.concept_full_name else null end)) as "Category",
GROUP_CONCAT(distinct ROUND(DATEDIFF(CURDATE(), obsForArtProg.value_datetime) / 7, 0)) as "Wks on ART",
piUIC.identifier as "UIC",
GROUP_CONCAT(distinct (case when personAttributeType.name = 'Mother\'s name' Then personAttribute.value else null end)) as "Mother's name",
GROUP_CONCAT(distinct (case when personAttributeType.name = 'District of Birth' then cvForRefferedFrom.concept_full_name else null end)) as "District of Birth",
GROUP_CONCAT(distinct (case when personAttributeType.name = 'Telephone' then personAttribute.value else null end)) as "Telephone no",
GROUP_CONCAT(
  distinct (
...
DATE(min(ordersForRegime.date_activated)) as "Date Of Initiation"


from patient
INNER JOIN obs obsForArtProg on patient.patient_id = obsForArtProg.person_id
INNER JOIN concept_name nameToGetArtProg on obsForArtProg.concept_id = nameToGetArtProg.concept_id and nameToGetArtProg.name = "PR, Start date of ART program" and nameToGetArtProg.concept_name_type = 'FULLY_SPECIFIED' AND obsForArtProg.voided=0
and obsForArtProg.person_id NOT in
(select obsForArtProg.person_id from obs obsForArtProg JOIN concept_name nameToGetArtProg on obsForArtProg.concept_id=nameToGetArtProg.concept_id and nameToGetArtProg.name = "PR, ART Program Stop Date" and nameToGetArtProg.concept_name_type = 'FULLY_SPECIFIED' and obsForArtProg.voided=0) /*Not to include patients which having stop date*/


INNER JOIN orders ordersForRegime on patient.patient_id = ordersForRegime.patient_id and ordersForRegime.order_type_id = 2 and date(ordersForRegime.date_activated) between date('2018-03-10') and date('2018-03-14')
and ordersForRegime.patient_id not in
(select patient_id from orders JOIN drug ON orders.concept_id = drug.concept_id
and drug.name in ("Tenofovir (TDF) 300mg + Lamivudine (3TC) 300mg + Efavirenz (EFV) 600mg",
...
"Indinavir 400mg",
"Saquinavir 200mg")
and date(orders.date_activated) < date('2018-03-10'))

INNER JOIN drug_order drugOrder on ordersForRegime.order_id = drugOrder.order_id
INNER JOIN drug  on drug.drug_id = drugOrder.drug_inventory_id and drug.name in
...
LEFT JOIN patient_identifier piOINo on patient.patient_id = piOINo.patient_id and piOINo.identifier_type in (select patient_identifier_type_id from patient_identifier_type where name = 'PREP/OI Identifier' and retired=0 and uniqueness_behavior = 'UNIQUE')
LEFT JOIN patient_identifier piUIC on patient.patient_id = piUIC.patient_id and piUIC.identifier_type in (select patient_identifier_type_id from patient_identifier_type where name = 'UIC' and retired=0 )
...
group by patient.patient_id, date(ordersForRegime.date_created);
