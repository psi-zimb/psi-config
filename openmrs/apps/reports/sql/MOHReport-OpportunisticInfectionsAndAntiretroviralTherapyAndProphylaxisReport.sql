...
SELECT/*Pivoting the table*/
    'D2. Number of new PLHIV in care in WHO Stage 1 at registration this month' AS '-',
    SUM(lessThan1yrMale) AS '<1 M',
    SUM(lessThan1yrFemale) AS '<1 F',
    SUM(1To9yrMale) AS '1-9 M',
    SUM(1To9yrFemale) AS '1-9 F',
    ...
    FROM
        ...
        AS D2NumberOfNewPLHIVinCareInWHOStage1AtRegistrationThisMonth
           INNER JOIN person p ON p.person_id = D2NumberOfNewPLHIVinCareInWHOStage1AtRegistrationThisMonth.person_id
           GROUP BY
           CASE
               WHEN timestampdiff(YEAR,p.birthdate,'#endDate#') < 1 AND p.gender = 'M'
               THEN '< 1 Yr M'
...
SELECT/*Pivoting the table*/
    'D3. Number of new PLHIV in care in WHO Stage 2 at registration this month' AS '-',
    SUM(lessThan1yrMale) AS '<1 M',
    SUM(lessThan1yrFemale) AS '<1 F',
    SUM(1To9yrMale) AS '1-9 M',
    SUM(1To9yrFemale) AS '1-9 F',
    ...
    FROM
        ...
        AS D3NumberOfNewPLHIVinCareInWHOStage2AtRegistrationThisMonth
           INNER JOIN person p ON p.person_id = D3NumberOfNewPLHIVinCareInWHOStage2AtRegistrationThisMonth.person_id
           GROUP BY
           CASE
               WHEN timestampdiff(YEAR,p.birthdate,'#endDate#') < 1 AND p.gender = 'M'
               THEN '< 1 Yr M'
...
SELECT/*Pivoting the table*/
    'D4. Number of new PLHIV in care in WHO Stage 3 at registration this month' AS '-',
    SUM(lessThan1yrMale) AS '<1 M',
    SUM(lessThan1yrFemale) AS '<1 F',
    SUM(1To9yrMale) AS '1-9 M',
    SUM(1To9yrFemale) AS '1-9 F',
    ...
    FROM
        ...
        AS D4NumberOfNewPLHIVinCareInWHOStage3AtRegistrationThisMonth
           INNER JOIN person p ON p.person_id = D4NumberOfNewPLHIVinCareInWHOStage3AtRegistrationThisMonth.person_id
           GROUP BY
           CASE
               WHEN timestampdiff(YEAR,p.birthdate,'#endDate#') < 1 AND p.gender = 'M'
               THEN '< 1 Yr M'
                ...
                INNER join patient_identifier artNumber
                        ...
                        And artNumber.identifier_type = (
                         ...
                         AND artNumber.identifier like '%-A-%'
                             ...
                             and drug.retired = 0
                             and date(ord.scheduled_date) between date('#startDate#') and date('#endDate#')
                             and artNumber.voided = 0
                             and COALESCE(date(artNumber.date_changed),date(artNumber.date_created)) <=date(ord.scheduled_date)
                             and (
                                  date(ord.date_stopped) is NULL 
                                  OR 
                                  date(ord.date_stopped) > date('#endDate#')
                                 )
