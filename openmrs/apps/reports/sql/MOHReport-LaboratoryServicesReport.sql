...
/*D19. Number of PLHIV in care on ART who Viral load test done less than (<1000 copies /mL) at 12 after ART initiation this month*/
SELECT/*Pivoting the table*/
    'D19. Number of PLHIV in care on ART who Viral load test done less than (<1000 copies /mL) at 12 after ART initiation this month' AS '-',
    SUM(lessThan1yrMale) AS '<1 M',
    SUM(lessThan1yrFemale) AS '<1 F',
    SUM(1To9yrMale) AS '1-9 M',
    SUM(1To9yrFemale) AS '1-9 F',
    ...
    FROM (
                ...
                and obsHIVRNAPCR.voided = 0
                and date(obsHIVRNAPCR.obs_datetime) between date('#startDate#') and date('#endDate#')
                and obsHIVRNAPCR.value_numeric < 1000
                and d.retired = 0
                and date(ord.date_activated) <= date('#endDate#')
         ) AS D19NumberofPLHIVincareonARTwhoViralloadtestdonelessthanthousandcopiesmLat12afterARTinitiationthismonth
           INNER JOIN person p ON p.person_id = D19NumberofPLHIVincareonARTwhoViralloadtestdonelessthanthousandcopiesmLat12afterARTinitiationthismonth.person_id
           GROUP BY
           CASE
               WHEN timestampdiff(YEAR,p.birthdate,'#endDate#') < 1 AND p.gender = 'M'
               THEN '< 1 Yr M'
...
/*D20. Number of PLHIV in care initiated on ART with a CD4 count < 350 cell/ mm3 this month*/
SELECT/*Pivoting the table*/
    'D20. Number of PLHIV in care initiated on ART with a CD4 count < 350 cell/ mm3 this month' AS '-',
    SUM(lessThan1yrMale) AS '<1 M',
    SUM(lessThan1yrFemale) AS '<1 F',
    SUM(1To9yrMale) AS '1-9 M',
    SUM(1To9yrFemale) AS '1-9 F',
    ...
    FROM (
            select distinct p.person_id
            from person p
            join obs obsCD4Count on p.person_id = obsCD4Count.person_id
            join concept_name cnCD4Count on obsCD4Count.concept_id = cnCD4Count.concept_id
            JOIN orders ord on p.person_id=ord.patient_id and ord.order_type_id = 2
                            JOIN drug_order dord on dord.order_id = ord.order_id 
                            JOIN drug d on dord.drug_inventory_id = d.drug_id
                            and d.name in   (
            ...
            and cnCD4Count.concept_name_type = 'FULLY_SPECIFIED'
            and cnCD4Count.voided = 0
            and obsCD4Count.voided = 0
            and d.retired = 0
            and date(obsCD4Count.obs_datetime) between date('#startDate#') and date('#endDate#')
            and obsCD4Count.value_numeric < 350
            and date(ord.date_activated) between date('#startDate#') and date('#endDate#')
            and ord.patient_id NOT In ( /*Excluding Patient if Any of the ARV Drug was given before reporting start date*/
                                            select person_id from person
                                            jOIN orders on person.person_id=orders.patient_id and orders.order_type_id = 2
         ...
         ) AS D20NumberofPLHIVincareinitiatedonARTwithaCD4countlessthan350cellmm3thismonth
           INNER JOIN person p ON p.person_id = D20NumberofPLHIVincareinitiatedonARTwithaCD4countlessthan350cellmm3thismonth.person_id
           GROUP BY
           CASE
               WHEN timestampdiff(YEAR,p.birthdate,'#endDate#') < 1 AND p.gender = 'M'
               THEN '< 1 Yr M'
...
/*D21. Number of PLHIV in care initiated on ART with a CD4 count 350- <500 cell/mm3 this month*/
SELECT/*Pivoting the table*/
    'D21. Number of PLHIV in care initiated on ART with a CD4 count 350- <500 cell/mm3 this month' AS '-',
    SUM(lessThan1yrMale) AS '<1 M',
    SUM(lessThan1yrFemale) AS '<1 F',
    SUM(1To9yrMale) AS '1-9 M',
    SUM(1To9yrFemale) AS '1-9 F',
    ...
    FROM (
            ...
            and d.retired = 0
            and date(obsCD4Count.obs_datetime) between date('#startDate#') and date('#endDate#')
            and obsCD4Count.value_numeric between 350 and 499
            and date(ord.date_activated) between date('#startDate#') and date('#endDate#')
            and ord.patient_id NOT In ( /*Excluding Patient if Any of the ARV Drug was given before reporting start date*/
                                            select person_id from person
                                            jOIN orders on person.person_id=orders.patient_id and orders.order_type_id = 2
         ...
         ) AS D21NumberofPLHIVincareinitiatedonARTwithaCD4countbetween350and500cellmm3thismonth
           INNER JOIN person p ON p.person_id = D21NumberofPLHIVincareinitiatedonARTwithaCD4countbetween350and500cellmm3thismonth.person_id
           GROUP BY
           CASE
               WHEN timestampdiff(YEAR,p.birthdate,'#endDate#') < 1 AND p.gender = 'M'
               THEN '< 1 Yr M'
...
/*D22. Number of PLHIV in care Initiated on ART with a Cd4 count >500 cell/mm3 this month*/
SELECT/*Pivoting the table*/
    'D22. Number of PLHIV in care Initiated on ART with a Cd4 count >500 cell/mm3 this month' AS '-',
    SUM(lessThan1yrMale) AS '<1 M',
    SUM(lessThan1yrFemale) AS '<1 F',
    SUM(1To9yrMale) AS '1-9 M',
    SUM(1To9yrFemale) AS '1-9 F',
    ...
    FROM (
            ...
            and d.retired = 0
            and date(obsCD4Count.obs_datetime) between date('#startDate#') and date('#endDate#')
            and obsCD4Count.value_numeric >=500
            and date(ord.date_activated) between date('#startDate#') and date('#endDate#')
            and ord.patient_id NOT In ( /*Excluding Patient if Any of the ARV Drug was given before reporting start date*/
                                            select person_id from person
                                            jOIN orders on person.person_id=orders.patient_id and orders.order_type_id = 2
         ...
         ) AS D22NumberofPLHIVincareInitiatedonARTwithaCd4countgreaterthanorqualto500cellmm3thismonth
           INNER JOIN person p ON p.person_id = D22NumberofPLHIVincareInitiatedonARTwithaCd4countgreaterthanorqualto500cellmm3thismonth.person_id
           GROUP BY
           CASE
               WHEN timestampdiff(YEAR,p.birthdate,'#endDate#') < 1 AND p.gender = 'M'
               THEN '< 1 Yr M'
