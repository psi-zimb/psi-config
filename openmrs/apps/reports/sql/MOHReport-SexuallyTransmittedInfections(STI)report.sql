...
/*F4. Total number of STI clients who tested positive for Syphilis this month*/

SELECT/*Pivoting the table*/
    'F4. Total number of STI clients who tested positive for Syphilis this month' AS '-',
    SUM(lessThan1yrMale) AS '<1 M',
    SUM(lessThan1yrFemale) AS '<1 F',
    SUM(1To9yrMale) AS '1-9 M',
    SUM(1To9yrFemale) AS '1-9 F',
...
/*F7. Total number of STI Clients with a known HIV positive status this month*/

SELECT/*Pivoting the table*/
    'F7. Total number of STI Clients with a known HIV positive status this month' AS '-',
    SUM(lessThan1yrMale) AS '<1 M',
    SUM(lessThan1yrFemale) AS '<1 F',
    SUM(1To9yrMale) AS '1-9 M',
    SUM(1To9yrFemale) AS '1-9 F',
    ...
    FROM (
             ...
             from obs obsForSTIDiagnosedPatients
                        INNER JOIN concept_set forSTIListOfDiagnosis
                                    on obsForSTIDiagnosedPatients.value_coded=forSTIListOfDiagnosis.concept_id
                                    and forSTIListOfDiagnosis.concept_set=(    /*list of Coded Diagnosis*/
                                                                                 select concept_id 
                                                                                 from concept_view 
                                                                                 where concept_full_name='Sexually Transmitted Infections'
                                                                                 AND retired=0
                        ...
                        where obsForSTIDiagnosedPatients.person_id in ( 
                          ...
                          or obsForSTIDiagnosedPatients.person_id in (
                                                                        ...
                                                                        from patient_identifier patientHavingOINumber
                                                                        where patientHavingOINumber.identifier_type =(
                                                                                                                        select 
...
/*F8. Number of clients given information on condom use*/
SELECT/*Pivoting the table*/
    'F8. Number of clients given information on condom use' AS '-',
    SUM(lessThan1yrMale) AS '<1 M',
    SUM(lessThan1yrFemale) AS '<1 F',
    SUM(1To9yrMale) AS '1-9 M',
    SUM(1To9yrFemale) AS '1-9 F',
    ...
    FROM ( 
       ...
       and date(obsForSTIDiagnosedPatients.obs_datetime) between date('#startDate#') and date('#endDate#') 
       and obsForSTIDiagnosedPatients.voided=0
            ) AS F8Numberofclientsgiveninformationoncondomuse
           INNER JOIN person p ON p.person_id = F8Numberofclientsgiveninformationoncondomuse.person_id
           GROUP BY
           CASE
               WHEN timestampdiff(YEAR,p.birthdate,'#endDate#') < 1 AND p.gender = 'M'
               THEN '< 1 Yr M'
...
/*F9. Total number of STI contacts treated this month*/
SELECT/*Pivoting the table*/
    'F9. Total number of STI contacts treated this month' AS '-',
    SUM(lessThan1yrMale) AS '<1 M',
    SUM(lessThan1yrFemale) AS '<1 F',
    SUM(1To9yrMale) AS '1-9 M',
    SUM(1To9yrFemale) AS '1-9 F',
    ...
    FROM ( 
            select distinct obsForSTIDiagnosedPatients.person_id,
                   obsForSTIDiagnosedPatients.value_coded
                   from  obs obsForSTIDiagnosedPatients
                   INNER JOIN concept_view cvforSTIListOfDiagnosis on obsForSTIDiagnosedPatients.value_coded = cvforSTIListOfDiagnosis.concept_id
                   INNER JOIN concept con on obsForSTIDiagnosedPatients.value_coded = con.concept_id
                   where cvforSTIListOfDiagnosis.concept_full_name IN /*List of Coded Diagnosis*/
                                                           (    "Chancroid contact",
                                                                "Gonorrhea Contact",
                                                                "Syphilis contact",
                                                                "Trichomoniasis contact",
                                                                "Urethral Discharge Syndrome Contact",
                                                                "Vaginal discharge syndrome Contact"
                                                           )
                  and con.class_id = 4
                  and con.retired = 0
                  and cvforSTIListOfDiagnosis.retired = 0                                         
                  and obsForSTIDiagnosedPatients.voided = 0
                  and obsForSTIDiagnosedPatients.concept_id = 15
                  and date(obsForSTIDiagnosedPatients.obs_datetime) between date('#startDate#') and date('#endDate#')
            ) AS F9TotalnumberofSTIcontactstreatedthismonth 
           INNER JOIN person p ON p.person_id = F9TotalnumberofSTIcontactstreatedthismonth.person_id
           GROUP BY
           CASE
               WHEN timestampdiff(YEAR,p.birthdate,'#endDate#') < 1 AND p.gender = 'M'
               THEN '< 1 Yr M'
