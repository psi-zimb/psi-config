...
and encounter_id=(select max(en.encounter_id) from encounter en
                  inner join obs observation on en.patient_id=patient_id
                  inner join concept_view concept
                             on concept.concept_id = observation.concept_id
                  where concept.concept_full_name like '%IPV Form%' and concept.retired=0
                  order by obs_datetime desc) order by sort_weight
